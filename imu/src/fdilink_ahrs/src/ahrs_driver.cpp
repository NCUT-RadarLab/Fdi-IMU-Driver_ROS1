...
namespace FDILink
{
  ahrsBringup::ahrsBringup() : frist_sn_(false), serial_timeout_(20)
  {
    ...
    pravite_nh.param("raw_gnss_topic_", raw_gnss_topic_, std::string("/raw_gnss"));
    ...
    raw_gnss_pub_ = nh_.advertise<sensor_msgs::NavSatFix>(raw_gnss_topic_.c_str(), 10);
  ...
  void ahrsBringup::processLoop() // 数据处理过程
  {
    ...
    while (ros::ok())
    {
      ...
      // 检查FDlink协议是否有对应驱动
      // TODO:加协议需要在这加上去
      if (head_type[0] != TYPE_RAW_GNSS && head_type[0] != TYPE_IMU && head_type[0] != TYPE_AHRS && head_type[0] != TYPE_INSGPS && head_type[0] != TYPE_GEODETIC_POS && head_type[0] != 0x50 && head_type[0] != TYPE_GROUND && head_type[0] != 0xff)
      {
      ...
      // 分类检查协议数据长度
      // TODO: 加新协议
      if (head_type[0] == TYPE_RAW_GNSS && check_len[0] != RAW_GNSS_LEN)
      {
        ROS_WARN("head_len error (raw_gnss)");
        continue;
      }
      ...
      else if (head_type[0] == TYPE_RAW_GNSS)
      {
        Raw_GNSS_frame_.frame.header.header_start = check_head[0];
        Raw_GNSS_frame_.frame.header.data_type = head_type[0];
        Raw_GNSS_frame_.frame.header.data_size = check_len[0];
        Raw_GNSS_frame_.frame.header.serial_num = check_sn[0];
        Raw_GNSS_frame_.frame.header.header_crc8 = head_crc8[0];
        Raw_GNSS_frame_.frame.header.header_crc16_h = head_crc16_H[0];
        Raw_GNSS_frame_.frame.header.header_crc16_l = head_crc16_L[0];
        uint8_t CRC8 = CRC8_Table(Raw_GNSS_frame_.read_buf.frame_header, 4);
        if (CRC8 != Raw_GNSS_frame_.frame.header.header_crc8)
        {
          ROS_WARN("header_crc8 error");
          continue;
        }
        if (!frist_sn_)
        {
          read_sn_ = Raw_GNSS_frame_.frame.header.serial_num - 1;
          frist_sn_ = true;
        }

        ahrsBringup::checkSN(TYPE_RAW_GNSS);
      }
      ...
      {
        uint16_t head_crc16_l = Raw_GNSS_frame_.frame.header.header_crc16_l;
        uint16_t head_crc16_h = Raw_GNSS_frame_.frame.header.header_crc16_h;
        uint16_t head_crc16 = head_crc16_l + (head_crc16_h << 8);
        size_t data_s = serial_.read(Raw_GNSS_frame_.read_buf.read_msg, (RAW_GNSS_LEN + 1)); //
        uint16_t CRC16 = CRC16_Table(Raw_GNSS_frame_.frame.data.data_buff, RAW_GNSS_LEN);
        if (if_debug_)
        {
          std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec << std::endl;
          std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec << std::endl;
        ...
        if (head_crc16 != CRC16)
        {
          ROS_WARN("check crc16 faild(raw_gnss).");
          continue;
        }
        else if (Geodetic_Position_frame_.frame.frame_end != FRAME_END)
        {
      ...
      {
        sensor_msgs::NavSatFix raw_gnss_data;
        raw_gnss_data.header.stamp = ros::Time::now();
        raw_gnss_data.header.frame_id = "raw_gnss";
        raw_gnss_data.latitude = Raw_GNSS_frame_.frame.data.data_pack.Latitude / DEG_TO_RAD;
        raw_gnss_data.longitude = Raw_GNSS_frame_.frame.data.data_pack.Longitude / DEG_TO_RAD;
        raw_gnss_data.altitude = Raw_GNSS_frame_.frame.data.data_pack.Height;
        // raw_gnss_data.position_covariance[0] = Raw_GNSS_frame_.frame.data.data_pack.Latitude_standard_deviation;
        // raw_gnss_data.position_covariance[1] = Raw_GNSS_frame_.frame.data.data_pack.Longitude_standard_deviation;
        // raw_gnss_data.position_covariance[2] = Raw_GNSS_frame_.frame.data.data_pack.Height_standard_deviation;

        raw_gnss_pub_.publish(raw_gnss_data);
      }
  ...
  void ahrsBringup::checkSN(int type)
  {
    ...
    case TYPE_RAW_GNSS:
      if (++read_sn_ != Raw_GNSS_frame_.frame.header.serial_num)
      {
        if (Raw_GNSS_frame_.frame.header.serial_num < read_sn_)
        {
          sn_lost_ += 256 - (int)(read_sn_ - Raw_GNSS_frame_.frame.header.serial_num);
          if (if_debug_)
          {
            ROS_WARN("detected sn lost.");
          }
        ...
        {
          sn_lost_ += (int)(Raw_GNSS_frame_.frame.header.serial_num - read_sn_);
          if (if_debug_)
          {
            ROS_WARN("detected sn lost.");
          }
      ...
      read_sn_ = Raw_GNSS_frame_.frame.header.serial_num;
      break;
