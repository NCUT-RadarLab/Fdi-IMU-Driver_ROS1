...
#pragma pack(1)
	...
	struct read_Raw_GNSS_struct
	{
		fdilink_header header; // 7
		union data
		{
			Raw_GNSS_Packet_t data_pack;
			uint8_t data_buff[74]; // 74
		} data;
		uint8_t frame_end; // 1
	};

	struct read_Raw_GNSS_tmp
	{
		uint8_t frame_header[7];
		uint8_t read_msg[75];
	};
	...
	union Raw_GNSS_frame_read
	{
		...
		read_Raw_GNSS_tmp read_buf;
		uint8_t read_tmp[82]; // 75+7=
	};
#pragma pack()
